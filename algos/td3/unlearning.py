...
def evaluate_unlearning(
    ...
    with torch.no_grad():
        ...
        # Get current actions for forget states
        current_forget_actions = agent.policy.actor(forget_states_tensor)
        ...
        if retain_states is not None:
            ...
            current_retain_actions = agent.policy.actor(retain_states_tensor)
    ...
    print(f"  Distance to original forget actions: {metrics['forget_mean_distance']:.4f} ± {metrics['forget_std_distance']:.4f}")
...
def unlearn_continuous_policy_kl(
    ...
    if hasattr(agent, "policy"):
        ...
        # Check for TD3 (deterministic actor)
        if hasattr(policy, "actor") and hasattr(policy, "actor_target"):
            algorithm_type = "TD3"
            actor_network = policy.actor
            print("Detected TD3 agent - will add noise for KL computation")
    ...
    def get_action_distribution(states):
        ...
        if algorithm_type == "TD3":
            # TD3 is deterministic, so we add small noise to create a distribution
            mean = actor_network(states)
            # Add small std for KL computation
            std = torch.ones_like(mean) * 0.1 * temperature1
    ...
    if method == 'random':
        ...
        target_std = torch.ones_like(forget_actions_tensor) * 0.1 * temperature2
