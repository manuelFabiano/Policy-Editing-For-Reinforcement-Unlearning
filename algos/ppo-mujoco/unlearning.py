...
def evaluate_unlearning(
    ...
    with torch.no_grad():
        ...
        current_forget_actions, _ = agent.predict(forget_states, deterministic=True)
        current_forget_actions = torch.tensor(current_forget_actions, device=device)
        ...
        if target_forget_means is not None:
            ...
            dist_forget_target = compute_normalized_distance(current_forget_actions, target_forget_means, agent.action_space, forget_actions=forget_actions_tensor)
        ...
        if retain_states is not None:
            ...
            current_retain_actions, _ = agent.predict(retain_states, deterministic=True)
            current_retain_actions = torch.tensor(current_retain_actions, device=device)
            ...
            if target_retain_means is not None:
                #dist_retain_target = torch.norm(current_retain_actions - target_retain_means, dim=-1)
                dist_retain_target = compute_normalized_distance(current_retain_actions, target_retain_means, agent.action_space)
...
def unlearn_continuous_policy_kl(
    ...
    if hasattr(agent, "policy"):
        ...
        # Check for PPO
        if hasattr(policy, "action_net") and hasattr(policy, "action_dist"):
            algorithm_type = "PPO"
            actor_network = policy.action_net
            ...
            for param in policy.action_net.parameters():
                ...
                ppo_actor_params.append(param)
            
            # Unfreeze mlp_extractor
            if hasattr(policy, "mlp_extractor"):
                # Option A: Unfreeze ALL mlp_extractor
            ...
            if hasattr(policy, "value_net"):
                for param in policy.value_net.parameters():
                    param.requires_grad = False  # Double safety
                    
            print(f"Detected PPO agent - training {len(ppo_actor_params)} actor parameters")
            print(f"Total frozen parameters: {sum(not p.requires_grad for p in policy.parameters())}")
    ...
    actor_network.train()
    # Enable gradients
    params_to_train = ppo_actor_params
    ...
    def get_action_distribution(states):
        if algorithm_type == "PPO":
            ...
            # Get distribution
            if isinstance(policy.action_dist, DiagGaussianDistribution):
    ...
    if retain_states is not None:
        ...
        with torch.no_grad():
            ...
            # Store only the mean for MSE loss (as in the discrete version)
    ...
    # Setup optimizer
    if algorithm_type == "PPO":
        optimizer = torch.optim.Adam(ppo_actor_params, lr=lr)
