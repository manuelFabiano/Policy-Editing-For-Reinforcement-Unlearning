...
def unlearn_policy_kl(agent, forget_states, forget_actions, retain_states=None, retain_actions=None,
    ...
    if hasattr(agent, "policy") and hasattr(agent.policy, "action_net"):
    ...
    if retain_states is not None:
        ...
        with torch.no_grad():
            ...
            # Save logits or probabilities, not the distribution object
...
def evaluate_unlearning(agent, forget_states, forget_actions, retain_states=None, retain_actions=None,
    ...
    def get_action_probs(states):
        ...
        with torch.no_grad():
            try:
                forget_distributions = agent.policy.get_distribution(states_tensor)
                probs = forget_distributions.distribution.probs
                return probs
            except Exception as e:
    ...
    try:
        ...
        # Fix: Ensure indices are integers and valid
        ...
        if target_forget_probs is not None:
            ...
            # Forget distance accuracy: percentage with normalized distance > threshold
            metrics['forget_distance_accuracy'] = (forget_distances > distance_threshold).float().mean().item() * 100
            metrics['forget_above_threshold'] = (forget_distances > distance_threshold).sum().item()
        ...
        if target_forget_probs is not None:
            ...
            print(f"    Distance Accuracy: {metrics['forget_distance_accuracy']:.1f}% ({metrics['forget_above_threshold']}/{metrics['forget_total']} > {distance_threshold})")
        ...
        if retain_states is not None and retain_actions is not None:
            ...
            if len(retain_action_probs) > 0:
                ...
                # NEW: Calculate normalized distance from original probabilities
                if original_retain_probs is not None:
                    ...
                    # Retain distance accuracy: percentage with normalized distance < threshold
                    metrics['retain_distance_accuracy'] = (retain_distances < distance_threshold).float().mean().item() * 100
                    metrics['retain_below_threshold'] = (retain_distances < distance_threshold).sum().item()
                    ...
                    print(f"    Distance Accuracy: {metrics['retain_distance_accuracy']:.1f}% ({metrics['retain_below_threshold']}/{metrics['retain_total']} < {distance_threshold})")
