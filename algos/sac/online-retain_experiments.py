...
def load_sac_from_hub(env_id="Walker2d-v3", env=None):
    """Load the SAC model directly from Hugging Face."""
    # Define the model path on Hugging Face
    repo_id = f"sb3/sac-{env_id}"
    filename = f"sac-{env_id}.zip"  # Standard filename for SAC models on Hugging Face
        ...
        for mode in modes:                       
            for states_n in trajectories_len:    
                for rep in range(1, repetitions + 1):  
                    ...
                    agent = load_sac_from_hub(env_id=env_id, env=env)
...
if __name__ == "__main__":
    ...
    run_unlearning(env_id="HalfCheetah-v3", param_grid={
            ...
            "retain_dim": [150],
            "retain_rand": [0.1],
            "temperature": [2.5]},output_path="./halfcheetah-final-seed_results.json", repetitions=10)
    run_unlearning(env_id="Hopper-v3", param_grid={
            ...
            "retain_dim": [300],
            "retain_rand": [0.05],
            "temperature": [5]}, output_path="./hopper-final-seed_results.json", repetitions=10)
    """
    run_unlearning(env_id="Walker2d-v3", param_grid={
            ...
            "retain_dim": [150],
            "retain_rand": [0.1],
            "temperature": [2.5]}, output_path="./walker2d-final-seed-random_results.json", repetitions=10)